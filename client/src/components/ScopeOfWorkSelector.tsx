...
interface ScopeOfWorkSelectorProps {
  ...
  onScopeDetailChange: (scopeId: string, description: string) => void;
}
...
const ScopeOfWorkSelector: React.FC<ScopeOfWorkSelectorProps> = ({
  ...
  const getScopeDetail = (scopeId: string) => {
    return (
      scopeDetails.find((detail) => detail.id === scopeId)?.description || ""
    );
  };
                  ...
                  <div className="flex items-center space-x-2 mb-1">
                    ...
                    <h4
                      className={`font-medium ${
                        isSelected ? "text-[#e30613]" : "text-gray-900"
                      }`}
                    >
                      {scope.name}
                    </h4>
            ...
            {isSelected && (
              <div className="ml-8 mr-4">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Describe your specific requirements for {scope.name} *
                </label>
                <textarea
                  value={getScopeDetail(scope.id)}
                  onChange={(e) =>
                    onScopeDetailChange(scope.id, e.target.value)
                  }
                  onClick={(e) => e.stopPropagation()}
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#e30613] focus:border-[#e30613] h-20 resize-none"
                  placeholder={`Please provide specific details about your ${scope.name.toLowerCase()} requirements (e.g., materials, scale, frequency, specific challenges, etc.)`}
                />
                <p className="text-xs text-gray-500 mt-1">
                  This information helps our AI provide more accurate tool
                  recommendations
                </p>
              </div>
            )}
