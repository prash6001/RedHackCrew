// Simple integration layer for enhanced FleetFlow recommendations
import { ProjectData, ToolRecommendation } from '../types/ProjectData';

// Enhanced tool recommendation with Hilti Fleet Management insights
...
export const enhanceRecommendations = (
  ...
  return baseRecommendations.map(tool => {
    ...
    const getTodSuitability = (): 'excellent' | 'good' | 'limited' | 'not_suitable' => {
    ...
    const getArchetypeMatch = (): 'essential' | 'recommended' | 'situational' => {
      ...
      const isPowerTool = ['drilling', 'cutting', 'fastening', 'demolition'].some(cat => 
...
export const generateQuickInsights = (projectData: ProjectData, recommendations: ToolRecommendation[]) => {
  ...
  const insights = {
    ...
    criticalTools: recommendations.filter(tool => 
      ['drilling', 'cutting', 'safety'].some(cat => tool.category.toLowerCase().includes(cat))
    ).length,
...
export const analyzeToolsOnDemand = (recommendations: ToolRecommendation[], projectData: ProjectData) => {
...
export const assessProjectRisks = (projectData: ProjectData, recommendations: ToolRecommendation[]) => {
  const risks = [];
...
export const getEnhancedAnalysis = (projectData: ProjectData, baseRecommendations: ToolRecommendation[]) => {
  return {
