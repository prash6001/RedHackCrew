import { getArchetypeTools, getToolNecessity, getArchetypeQuantityMultiplier, getArchetypeJustification } from './projectArchetypes';
import { enhanceToolWithServiceFeatures, evaluateToolsOnDemand } from './fleetServiceModeling';
...
export class EnhancedRecommendationEngine {
    static generateRecommendationStrategy(projectData) {
        // Step 1: Get archetype-based tool requirements
        const archetypeTools = getArchetypeTools(projectData.projectType);
        ...
        const strategicInsights = this.generateStrategicInsights(projectData, coreFleetTools, todCandidates, tcoAnalysis, riskProfile);
        return {
            coreFleetTools,
    ...
    static generateBaseRecommendations(projectData, archetypeTools) {
        ...
        const quantityMultiplier = getArchetypeQuantityMultiplier(projectData.projectType, projectData.laborCount);
        relevantTools.forEach(tool => {
            ...
            const necessity = getToolNecessity(tool.name, projectData.projectType);
            const justification = [
                ...getArchetypeJustification(tool.name, projectData.projectType, necessity),
    ...
    static evaluateToolsOnDemand(recommendations, projectData) {
        const todCandidates = [];
        recommendations.forEach(tool => {
            if (tool.todEligible) {
                ...
                scenarios.forEach(scenario => {
                    if (scenario.savings > tool.totalCost * 0.15) { // 15% savings threshold
    ...
    static analyzeToD_Scenarios(tool, projectData) {
        const scenarios = [];
        ...
        if (!baseTool?.todEligibility.eligible)
            return scenarios;
        ...
        if (specialtyMonths >= baseTool.todEligibility.minDuration) {
            ...
            if (specialtyScenario)
                scenarios.push(specialtyScenario);
        }
    ...
    static createHybridStrategies(recommendations, projectData) {
        const strategies = [];
        recommendations.forEach(tool => {
    ...
    static finalizeCoreFleet(recommendations, todCandidates) {
    ...
    static generateServiceJustification(tool) {
    ...
    static generateCompetitiveAdvantages(tool) {
        return [
    ...
    static generateStrategicInsights(projectData, coreFleet, todCandidates, tcoAnalysis, riskProfile) {
        const insights = [];
