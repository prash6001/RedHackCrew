...
export const calculateTCO = (projectData, recommendations, managerHourlyRate = INDUSTRY_BENCHMARKS.MANAGER_HOURLY_RATE) => {
...
const calculateTheftRisk = (recommendations, totalAssetValue) => {
    ...
    recommendations.forEach(tool => {
        ...
        const categoryProbability = INDUSTRY_BENCHMARKS.THEFT_PROBABILITY[tool.category.toLowerCase()] ||
...
const generateKeyFactors = (projectData, ownership, fleet, savings) => {
    const factors = [];
...
export const generateDetailedBreakdown = (tcoComparison, projectTimeline = 48) => {
...
export const assessProjectRisk = (projectData, tcoComparison) => {
    const risks = [];
...
//# sourceMappingURL=tcoCalculator.js.map