export class SystemValidator {
    validationResults = [];
    async runComprehensiveValidation(projectData) {
    ...
    async validateTCOCalculator(projectData) {
        ...
        try {
            ...
            const tcoResult = calculateTCO(projectData, mockRecommendations);
            // Validate structure
            tests.push({
    ...
    async validateRecommendationEngine(projectData) {
        const startTime = performance.now();
        const tests = [];
        try {
    ...
    async validateRiskModeling(projectData) {
        const startTime = performance.now();
        const tests = [];
        try {
    ...
    async validateServiceModeling(projectData) {
        const tests = [];
        try {
    ...
    async validateProjectArchetypes() {
        const tests = [];
        try {
    ...
    async validateDataIntegrity(projectData) {
        const tests = [];
    ...
    async validatePerformance(projectData) {
        const tests = [];
        const startTime = performance.now();
        try {
            const { EnhancedRecommendationEngine } = await import('./enhancedRecommendationEngine');
    ...
    validateProjectDataStructure(data) {
    ...
    generateValidationSummary() {
        ...
        const passedTests = this.validationResults.reduce((sum, result) => sum + result.tests.filter(test => test.status === 'PASS').length, 0);
        const failedTests = this.validationResults.reduce((sum, result) => sum + result.tests.filter(test => test.status === 'FAIL').length, 0);
        const warningTests = this.validationResults.reduce((sum, result) => sum + result.tests.filter(test => test.status === 'WARNING').length, 0);
        return {
...
export const runSystemValidation = async (projectData) => {
