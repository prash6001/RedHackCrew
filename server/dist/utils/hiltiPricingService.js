...
class HiltiPricingService {
  ...
  `;
     ...
     */
    async fetchPrices(products, country = 'US', language = 'en') {
        try {
            ...
            return streetPrices.map((price) => this.transformPriceData(price));
        }
        catch (error) {
     ...
     */
    async fetchSinglePrice(productId, quantity = 1) {
     ...
     */
    static getProductIdFromCatalogItem(catalogItem) {
     ...
     */
    async fetchPricesFromCatalog(products, catalogData) {
        if (!catalogData) {
        ...
        catalogData.forEach(category => {
            category.products?.forEach((product) => {
                if (product.apiProductIds) {
                    product.apiProductIds.forEach((id) => {
     ...
     */
    async fetchPricesWithRetry(products, maxRetries = 2, catalogData) {
        let lastError;
...
//# sourceMappingURL=hiltiPricingService.js.map