...
import { generateEnhancedRecommendations } from './utils/recommendationEngine.js';
import { calculateProjectMetrics, generateAccurateFleetContract } from './utils/accurateCalculations.js';
const app = express();
...
app.post('/api/analyze', upload.single('blueprint'), async (req, res) => {
    try {
        ...
        try {
            ...
            if (req.file) {
                projectData.blueprint = req.file; // Multer file object
...
app.post('/api/proposal', async (req, res) => {
    try {
        ...
        const termMultiplier = termMultipliers[selectedTerm];
        // Calculate adjusted costs
        const adjustedRecommendations = recommendations.map((tool) => ({
            ...tool,
        ...
        const proposal = {
            ...
            contractOptions: [
                {
                    term: 12,
                    monthlyRate: Math.round(adjustedRecommendations.reduce((sum, tool) => sum + tool.monthlyCost, 0)),
                    setupFee: 500,
                    totalCost: Math.round(adjustedRecommendations.reduce((sum, tool) => sum + tool.monthlyCost, 0) * 12 + 500),
                    savings: 'Standard',
                    benefits: ['Standard maintenance', 'Basic support', 'Equipment replacement']
                },
                {
                    term: 24,
                    monthlyRate: Math.round(adjustedRecommendations.reduce((sum, tool) => sum + tool.monthlyCost, 0) * 0.85),
                    setupFee: 250,
                    totalCost: Math.round(adjustedRecommendations.reduce((sum, tool) => sum + tool.monthlyCost, 0) * 0.85 * 24 + 250),
                    savings: 'Enhanced',
                ...
                {
                    term: 36,
                    monthlyRate: Math.round(adjustedRecommendations.reduce((sum, tool) => sum + tool.monthlyCost, 0) * 0.75),
                    setupFee: 0,
                    totalCost: Math.round(adjustedRecommendations.reduce((sum, tool) => sum + tool.monthlyCost, 0) * 0.75 * 36),
                    savings: 'Maximum',
